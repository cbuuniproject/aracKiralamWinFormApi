...
{
	public partial class frm_RolListele : Form
	{
		public frm_RolListele()
		{
			InitializeComponent();
		}
		...
		private async void button1_ClickAsync(object sender, EventArgs e)
		{
			try
			{
				...
				using (var client = new HttpClient())
				{
					...
					using (var result = await client.GetAsync("api/Rol"))
					{
						// Check the Result
						if (result.IsSuccessStatusCode)
						{
							...
							dg_Listele.DataSource =
								JsonConvert.DeserializeObject<ResponseContent<Rol>>(value).Data.ToList();
						}
					}
				}
			}
